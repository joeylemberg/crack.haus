var Trees = {
	...
	makeTree: function(x,y,size, branchColor){
		var tree = {
			branches: [],
			leaves: [],
...
//			theta = (root.theta + theta) / 2;
		...
		if(branch.depth == 0){
			...
			branch.branches.push(Trees.newBranch(root, branch, branch.tip.x, branch.tip.y, branch.size * 0.8, theta + Math.random() * 2));
			branch.branches.push(Trees.newBranch(root, branch, branch.tip.x, branch.tip.y, branch.size * 0.8, theta + Math.random() * 2 - 2));
		}
		return branch;
	...
	drawBranch: function(branch){
		ctx.beginPath();
		ctx.lineWidth = Math.ceil(branch.size * 0.45);
